...
export function TicketThread({
  ...
  const { data, loading, error } = useApi<{ ticket: TicketDetail }>(
    token ? `/tickets/${ticketId}?_r=${reload}` : null,
    token ?? undefined,
  );
  ...
  async function reply(e: FormEvent) {
    ...
    setBusy(true);
    try {
      await api.post(`/tickets/${ticketId}/messages`, { body }, token);
      setBody("");
      setReload((r) => r + 1);
    } catch (err) {
  ...
  return (
    <Container className="max-w-2xl space-y-5 py-10">
      <Link href={`/${locale}/support`} className="text-sm text-muted hover:text-foreground">
        ← {t.back}
      </Link>
          ...
          <div key={m.id} className={cn("flex", m.isStaff ? "justify-start" : "justify-end")}>
            <div
              className={cn(
                ...
                m.isStaff ? "bg-surface" : "bg-primary/10",
              )}
            >
